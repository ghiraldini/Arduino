...
Rx64Response rx64 = Rx64Response(); // declare response
...
unsigned long get_date(){            
  ...
  time_date[3] = day;
  time_date[4] = year;
  time_date[5] = day_of_week;//get rid of
} 
...
//  XBeeAddress64 addr64 = XBeeAddress64(0x0013A200, 0x402CE5A2);// 64 bit addressing (MY Xbee Radios)
  ...
  if (xbee.getResponse().isAvailable()){  //-----change from here-----    
    ...
    if (xbee.getResponse().getApiId() == TX_STATUS_RESPONSE){// got a response, should be a tx status (zb for series 2)
      Serial.println("TX_STATUS_RESPONSE");   
      xbee.getResponse().getTxStatusResponse(txStatus);// send transmission status response (series 1)
      Serial.print("TX Status = ");
      Serial.println(TX_STATUS_RESPONSE, HEX); // 0x89
      //Serial.print("Last File Sent = ");
      //Serial.println(last_file_sent);
      if (txStatus.getStatus() == SUCCESS){ // get status response (command only for series 1)
