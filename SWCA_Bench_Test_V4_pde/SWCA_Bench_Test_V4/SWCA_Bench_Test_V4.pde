...
// Change these parameters to modify behavior of system
  ...
  unsigned long xbWait =  90000;                                             // milliseconds to wait for response from XBee/LabVIEW  
  ...
  int systemMaint = 30;                                                      // digital pin for system maintenance mode switch
...
// -------------------------------------------------------------------------------- XBee radio vars
  ...
  uint8_t TX_payload[32];                                                               // results from polling inverter  
...
//  XBeeResponse response = XBeeResponse();                                               // Declare response for Series 1 packet type
...
// -------------------------------------------------------------------------------- Jason's vars
  ...
  uint8_t f_data[31];                               // array to save data from file
...
void loop() {
    ...
    if(!digitalRead(systemMaint)) {      // Check to see if maintenance switch is enabled
      digitalWrite(maintLED, HIGH);
      maintMode = 1;
      if(debug) {                               // *** debug code  
        Serial.println("Maintenance Mode Active");
      }      
//      maintenance_mode(maintMode);              // send alert to Base Station
      while(!digitalRead(systemMaint)) {  // loop until maintenance mode switch is toggled
        delay(100);
      } 
...
//      maintenance_mode(maintMode);              // clear alert with Base Station
    }
...
//   *** try version below    if (ping_result == 1){//ping result = SUCCESS, send data
          ...
          if( ping_labview(sizeof(TX_payload)) ) {                           // make sure LabVIEW is online and responding
              ...
              if( debug ) {
                  Serial.println("LabVIEW ping SUCCESS");
                }  
          } else {
                if(debug) {
...
bool readLabVIEWPing(byte *sDat, int sDatLen) {               // General serial read routine for SWCA port
  ...
  if(waitForSerial3data()) {                                  // hangout till serial data arrives
    ...
    for(int i = 0; i < sDatLen; i++ ) {                       // Read bytes from serial port 3
     rcvByte = Serial3.read(); 
...
byte send_xbee(uint8_t payload[], int packet_size, int file_num, int last_file) {    
...
int ping_labview(int packet_size) {
  uint8_t ping[] = {0x02, 0x4F, 0x4B};                                       // payload - message type 2 for XBee API type 0x10 zbTx payload 
...
void maintenance_mode(int status){

  uint8_t maint_mode[] = {  0x03, 0x00   };      // at byte 17,18 in receive string
