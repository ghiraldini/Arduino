...
void initializeFileSystem() {
  ...
  unsigned long resetPressed;             //
  unsigned long resetHeld;                //
  
  windowStart = millis();
  do {
    if(digitalRead(!fileResetPin)) {                                // check to see if reset button is pressed, using inverse logic due to pullup config
       resetPressed = millis();
       resetHeld = resetPressed;
       while( resetPressed - resetHeld  < resetHoldPeriod) {      // wait for minimum time 
         delay(5);
       }
       if( digitalRead(!fileResetPin)) {                           // check to see if reset button is still pressed
          EEPROM.write( 1,1 );
          file_num = 1;
          initFileSystem = true;
          EEPROM.write( 2,0);
...
//      sendAlert();              // send alert to Base Station
      while(!digitalRead(systemMaintPin)) {  // loop until maintenance mode switch is toggled
        delay(100);
      } 
...
//  ZBTxStatusResponse txStatus = ZBTxStatusResponse();// transmit response packet 
  ...
  if (xbee.getResponse().isAvailable()) {                         // look for response, 
    Serial.println("Response avaialble");  
