...
unsigned long dateStampForUSB()  {              // Below required to reset the register address to 0.
  ...
  second = bcdToDec(Wire.receive() & 0x7f); // I have declared all these in here so that the
  minute = bcdToDec(Wire.receive());        // function is self contained
  hour = bcdToDec(Wire.receive() & 0x3f);   // you could declare them at the top and then get at
  day_of_week=bcdToDec(Wire.receive());     // the bits individually elsewhere in the code
  day = bcdToDec(Wire.receive());           // but this could make the code less logical and
  month = bcdToDec(Wire.receive());         // more difficult to debugg  
