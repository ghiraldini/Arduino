...
// ----------------------------------------------------------------------------------------------------------------------- Jason's vars
  ...
  bool catching_up;
  bool ping_result;                                  // status from labview ping
...
//  int vdip_rts = 34;                                // CTS of VDIP
  ...
  byte file_data[50];                                   // variable to read serial
...
//-------------------------PROBLEM----------------------?? Cause other stuff is commented out???--------------------------------------------
              if( debug ) {// resume here after ping & delete is done = file 1 sent, but not written to stick
          ...
          if(file_num < max_file_number)      // increment file number if not over limit
              {                                      
          ...
          if( (file_num == max_file_number && last_file == 0)             // delete files if over limit and all files have been sent
            || (file_num > 10 && last_file == 0) )
          {
            delete_files(file_num);      
            Serial.print("File Number after deleting: ");
            Serial.println(file_num);            
          }
          if( file_num == max_file_number        // do not increment if over limit and transmission is down
                     && last_file != 0)
          {
            file_num = file_num;                      
            EEPROM.write(1,file_num);
...
if( debug) {  
  ...
  if (file_num == max_file_number){
    Serial.println("Over file limit, Not writing to stick...");
    return;
  }
...
byte delete_files(int file_number1){
  if(last_file == 0){                           // only delete if all files have been sent
    ...
    Serial.print("Restart at file number: ");
    Serial.println(file_num);
    Serial.print("Last file now equal to: ");
    Serial.println(last_file);
    del_complete = true;
  } 
  return del_complete;
